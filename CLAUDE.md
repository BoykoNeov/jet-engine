...
published case. All rungs are live; the current scope is **rung 66**. Read a rung's spec (last
...
**Current scope (rung 66).** The **cycle solve** is a thermally-perfect, reacting,
...
`build_turbojet(…).run(…)` design run **bit-for-bit rung 6**. Rungs **31–66** are the
...
64: the φ-referenced LIMITER; 65: its LAGGED valve; 66: the two-lag CASCADE).** The table above is its one-line hook; the
...
- **CASCADE A** (rung 66's seam) — a lagged `Tt4` governor beside the valve: two loops on *different* variables, so rung 66's identity lapses and an oscillatory actuator mode is admissible. Also still open: an ASYMMETRIC valve, and THREE loops on one variable.
