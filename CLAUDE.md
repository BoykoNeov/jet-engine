...
anchored to a published case. All rungs are live; the current scope is **rung 47**.
...
**Current scope (rung 47).** The **cycle solve** is a thermally-perfect, reacting,
...
`build_turbojet(…).run(…)` design run **bit-for-bit rung 6**. Rungs **31–47** are the
...
topping governor on it, and rung 47's **response lag** `τ_gov` on that governor); rungs **7–30, 36, 41, 44, 45** are pure
...
- **The variable stator** (moves `φ_surge` itself — rung 42 did the bleed half); a **bleed schedule** `b(n_L)`; **fuel + bleed together**; a **lead-compensated / anticipatory topping governor** (rung 47 shut the *pure-lag* door — a trailing-edge lag cannot reach the early LP surge min; a governor with phase LEAD could, the one thing a lag cannot — plus a sensor+actuator cascade or a rate-limit).
...
- `tests/` — per-rung `test_rungN.py` (N = 1…47; plus the rung-1/2b/3/4/5 files). Every rung file
