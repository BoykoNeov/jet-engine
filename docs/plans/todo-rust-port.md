...
method bodies, none deferred.** `tests/slice_z_smoke.rs` grows from 9 gates to **18, green in
0.98 s**; step 1's four stub-reading gates are REWRITTEN as booked, not adjusted.
...
wrong in a way the bar could not show. **A ROUTE WITNESS IS NOT A RUNG WITNESS**: `key_count`
discriminates rung 52 from rung 65 from the two cascades, and cannot separate rung 46 from rung
47 at all. Both affected gates now take the DIRECT `integrate_fuel` route, which is the route
...
- **P2 — SIX ARMS ONTO FIVE TARGETS: GATED SIX-FOR-SIX.** rung 66→65, 67→65 (floored); 66/67→64
  (unfloored); 66→52; 67→66; 67→47. The count is stated because the noun is checkable and the
  previous commit was a tally that did not add up.
- **P6 — HELD.** The two IC solves are separate, and each body states the other's failure mode.
- **P9 — MEASURED, NOT MAPPED.** The whole smoke file is **0.98 s**; no `slow` marker is
  introduced.
