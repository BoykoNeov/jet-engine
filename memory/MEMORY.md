...
## The Rust port — decided 2026-08-12; **PHASES 0–5 DONE**; **PHASE 6 (transients) AUTHORISED 2026-08-17**, six slices P–U, **P, Q, R SHIPPED; S STEPS 1–4 OF 5 SHIPPED, 5 IS DOCS (§ 5.16); T, U remain**
...
- [Slice K: rungs 38/39](rust-port-slice-k.md) — the phase table's scope list had never been ENUMERATED: it dropped one rung and double-counted another
...
- [Slice S pre-flight](rust-port-slice-s-preflight.md) — an inherited IOU named a gas the code REFUSES; and a prediction whose justification already killed a neighbour's is a reason to measure, not to write
- [Slice S step 1: the port + smoke](rust-port-slice-s-step1.md) — the plan's table lost a row its own prose counted; two injections reporting "nothing moved" could not have moved anything
- [Slice S step 2: rung 43's gates](rust-port-slice-s-step2.md) — a NON-STRICT ordering assertion is satisfied by the variable going inert; and 1 of 11 gates caught a wrong LP derivative
