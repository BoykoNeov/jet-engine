...
- Slice AG (75/76) [pre-flight](rust-port-slice-ag-preflight.md) [step 1](rust-port-slice-ag-step1.md) [step 2](rust-port-slice-ag-step2.md) [step 3](rust-port-slice-ag-step3.md) — a line citation, a gate grid and an enumeration of a field's READERS all have an EXPIRY DATE, and each expires at the boundary its own defect class is born on (one lesson per file)
