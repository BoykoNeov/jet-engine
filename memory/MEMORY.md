...
## The Rust port — decided 2026-08-12; **PHASES 0–6 DONE** — phase 6 authorised 2026-08-17 and **COMPLETED 2026-08-20** with slice U's five steps; **PHASE 7 AUTHORISED 2026-08-20**, slice V (of V…AJ) pre-registered; steps 1a/1b/2/3 SHIPPED
...
- Slice V (57-60) [authorised + steps 1a/1b](rust-port-slice-v.md) [step 2](rust-port-slice-v-step2.md) [step 3](rust-port-slice-v-step3.md) — a census matches a SHAPE, so a bare permanent assignment was invisible to one built on `try/finally`; both gates written to CLOSE a step could not fail; an injection whose only trace is OBJECT STATE reads exactly like one that never applied, so a did-it-move probe needs witness keys no gate reads
