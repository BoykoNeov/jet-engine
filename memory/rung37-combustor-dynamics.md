...
metadata: 
  ...
  modified: 2026-07-21T16:50:14.452Z
---
...
makes `E = E(r, θ₀)` **history-dependent** (refutes rung 34's blanket claim). Surge is **PROTECTED**:
`cold < hot-reslam < adiabatic` (cold metal depresses `Tt4_turb` → colder NGV passes more corrected
flow → `φ` away from surge; channel a beats the rung-36 "slow spool parks at thin low-ν margin"
counter-channel, which doesn't flip the EARLY peak). So rung 34/35's adiabatic combustor is the
conservative **WORST case**. The cost is the **accel-time LAG** (cold ~2.5–3× slower — the primary
CRS/Walsh-Fletcher effect) and the **hot RESLAM** (bodie). Heat-soak equilibrium == rung 35 because
`Tm=Tt4_burner ⇒ Q=0` at steady (**transient-only — never moves the running line**).
