...
def print_state_coordinate_table(flight):
    ...
    print("    the six rungs the gap doc declined to claim about, NO SHIPPED march")
    print("    stands still: rungs 72/73/77 march in CLIP (77 at 0.80, this wall, and")
    print("    it is the LIVELIEST march in the family), rungs 75/76 in `demand` at")
    print("    0.76, and RUNG 74 -- the only one with an arrested arm at all -- sites")
    print("    its own shipped arms at 0.76/0.70, which move. And that cell was found,")
    print("    mechanised and GATED at RUNG 74 s 2.2 -- `THE ARREST, disclosed, not")
    print("    tuned away` -- before rungs 78/79 were built. Neither spec CITED it")
    print("    (rung 78 s 5.3 now records it); s 5 above marched inside it, so the")
    print("    hits/binds counters there read ONE point 341 times. Every number stands,")
    print("    the credit does not. The audit's own finding: at `(demand, 0.76)` the")
    print("    VALVE and the STATOR are INERT 0/341, so rungs 75/76 read a TWO-loop")
    print("    plant -- and a liveness counter on a FROZEN plant reports the initial")
    print("    condition at FULL count (valve = 341/341 here): spreads are read FIRST.")
